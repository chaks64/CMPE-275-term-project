...
//	public List<Event> myfunction(String location, String status, String startTime, String endtDate, String keyword, String organizer);
	@Query(value = "select * from event inner join user on event.user_id = user.user_id where lower(event.city) like :location and lower(event.status) like :status and event.start_date >= convert_tz(:startTime,'+08:00','+08:00') and event.endt_date <= convert_tz(:endtDate,'+08:00', '+08:00') and (lower(event.description) like :keyword or lower(event.title) like :keyword) and lower(user.screen_name) like :organizer", nativeQuery = true)
	public List<Event> myfunction(String location, String status, String startTime, String endtDate, String keyword, String organizer);
//=======
